...
export class EmployeeStore {
  async index(): Promise<Employee[]> {
    try {
      const response = await axios.get<Employee[]>(employeeUrl, connectDB);
  ...
  async getEmployeeByNo(employeeNumber: string): Promise<Employee> {
    const empUrl = `${employeeUrl}('${employeeNumber}')`;
    try {
      const response = await axios.get<Employee>(empUrl, connectDB);
      return response.data;
    } catch (err) {
      throw new Error(
